...
​		小型的外包项目，给客户A做一个产品展示网站，客户A的朋友感觉效果还不错，也希望做这样的产品展示网站，但是要求有些不同：
...
3. 方案设计示意图
   ...
   ![image-20211126110829052](设计模式.assets/image-20211126110829052.png)
...
4. 享元模式经典的应用场景就是池技术了，String常量池、数据库连接池、缓冲池等等都是享元模式的应用，享元模式是池技术的重要实现方式
   ...
   ![image-20211126112144081](设计模式.assets/image-20211126112144081.png)

享元模式的原理类图：

![image-20211126112207363](设计模式.assets/image-20211126112207363.png)

对类图的说明：

1. FlyWeight是抽象的享元角色，他是产品的抽象类，同时定义出对象 的外部状态和内部状态的接口或实现
...
2. 思路分析和图解（类图）
   ...
   ![image-20211126135321894](设计模式.assets/image-20211126135321894.png)
...
![image-20211126135818935](设计模式.assets/image-20211126135818935.png)
...
3. 代理模式有不同的形式，主要有三种 静态代理、动态代理（JDK代理、接口代理）和Cglib代理（可以在内存动态地创建对象，而不需要实现接口，它是属于动态代理的范畴）。

4. 代理模式示意图

   ![image-20211126153341615](设计模式.assets/image-20211126153341615.png)
...
2. 思路分析图解（类图）
   ...
   ![image-20211126153952352](设计模式.assets/image-20211126153952352.png)
...
![image-20211126161018669](设计模式.assets/image-20211126161018669.png)
...
![image-20211126161537330](设计模式.assets/image-20211126161537330.png)
...
![image-20211126161710425](设计模式.assets/image-20211126161710425.png)

3. 代码实现+Debug源码[待 debug]
...
![image-20211126171804543](设计模式.assets/image-20211126171804543.png)
...
1. 应用实例要求
   编写制作豆浆的程序，说明如下: • 制作豆浆的流程 选材--->添加配料--->浸泡--->放到豆浆机打碎 • 通过添加不同的配料，可以制作出不同口味的豆浆 • 选材、浸泡和放到豆浆机打碎这几个步骤对于制作每种口味的豆浆都是一样的(红豆、花生豆浆。。。)

2. 思路分析和图解(类图)

   ![image-20211126171930200](设计模式.assets/image-20211126171930200.png)
...
![image-20211126172417449](设计模式.assets/image-20211126172417449.png)

3. 针对源码的类图（说明层次关系）

   ![image-20211126172457973](设计模式.assets/image-20211126172457973.png)
...
4. 通俗易懂的理解：将军发布命令，士兵去执行。其中有几个角色：将军（命令发布者）、士兵（命令的具体执行者）、命令(连接将
    军和士兵)。Invoker是调用者（将军），Receiver是被调用者（士兵），MyCommand是命令，实现了Command接口，持有接收对象。

命令模式的原理类图：

![image-20211130135101314](设计模式.assets/image-20211130135101314.png)
...
2. 思路分析和图解
   ...
   ![image-20211130135218682](设计模式.assets/image-20211130135218682.png)
...
![image-20211130135757703](设计模式.assets/image-20211130135757703.png)
...
![image-20211130135940710](设计模式.assets/image-20211130135940710.png)
...
![image-20211130151607344](设计模式.assets/image-20211130151607344.png)
...
![image-20211130152109511](设计模式.assets/image-20211130152109511.png)
...
2. 思路分析和图解(类图)
   ...
   ![image-20211130152455798](设计模式.assets/image-20211130152455798.png)
...
​		单分派（single dispatch）的含义比较好理解，单分派（single dispatch）就是说我们在选择一个方法的时候仅仅需要根据消息接收者（receiver）的运行时型别（Run time type）。实际上这也就是我们经常提到的多态的概念（当然C++中的函数重载也是Sigle dispatch的一种实现方式）。

​		举一个简单的例子，我们有一个基类A，A有一个虚方法f（可被子类override），D1和D2是A的两个子类，在D1和D2中我们覆写（override）了方法f。这样我们对消息f的调用，需要根据接收者A或者A的子类D1/D2的具体型别才可以确定具体是调用A的还是D1/D2的f方法。double dispatch（双分派）则在选择一个方法的时候，不仅仅要根据消息接收者（receiver）的运行时型别（Run time type），还要根据参数的运行时型别（Run time type）。当然如果所有参数都考虑的话就是multi-dispatch（多分派）。

​		举一个简单的例子，同于上面单分派中例子，A的虚方法f带了一个C型别的参数，C也是一个基类，C有也有两个具体子类E1和E2。这样，当我们在调用消息f的时候，我们不但要根据接收者的具体型别（A、D1、D2）， 还要根据参数的具体型别（C、E1、E2），才可以最后确定调用的具体是哪一个方法f。

​		这里所说的消息就是指方法，消息接收者就是指调用方法的那个对象。
...
![image-20211130154600978](设计模式.assets/image-20211130154600978.png)
...
![image-20211130154738031](设计模式.assets/image-20211130154738031.png)
...
2. 设计思路分析
   ...
   ![image-20211130155013378](设计模式.assets/image-20211130155013378.png)
...
2. 代码分析+类图+说明
   ...
   ![image-20211130160024176](设计模式.assets/image-20211130160024176.png)

![image-20211130160039661](设计模式.assets/image-20211130160039661.png)
...
![image-20211130165844037](设计模式.assets/image-20211130165844037.png)

![image-20211130165856409](设计模式.assets/image-20211130165856409.png)
...
- 思路分析图解(类图)
  ...
  ![image-20211130170835544](设计模式.assets/image-20211130170835544.png)
...
![image-20211130171806828](设计模式.assets/image-20211130171806828.png)
...
![image-20211130173013122](设计模式.assets/image-20211130173013122.png)
...
![image-20211130173145237](设计模式.assets/image-20211130173145237.png)

对原理类图的说明-即(中介者模式的角色及职责)
1) Mediator 就是抽象中介者,定义了同事对象到中介者对象的接口
2) Colleague 是抽象同事类
3) ConcreteMediator 具体的中介者对象, 实现抽象方法, 他需要知道所有的具体的同事类,即以一个集合来管理HashMap，并接受某个同事对象消息，完成相应的任务
4) ConcreteColleague具体的同事类，会有很多，每个同事只知道自己的行为，而不了解其他同事类的行为（方法），但是它们都依赖中介者对象。
...
2. 思路分析和图解(类图)
   ...
   ![image-20211130173641394](设计模式.assets/image-20211130173641394.png)
...
![image-20211130174214480](设计模式.assets/image-20211130174214480.png)
...
![image-20211130174340175](设计模式.assets/image-20211130174340175.png)
...
2. 思路分析和图解(类图)
   ...
   ![image-20211130174626294](设计模式.assets/image-20211130174626294.png)
...
![image-20211201100648988](设计模式.assets/image-20211201100648988.png)
...
![image-20211201100834439](设计模式.assets/image-20211201100834439.png)
...
2. 思路分析和图解(类图)
   ...
   ![image-20211201100930627](设计模式.assets/image-20211201100930627.png)
...
2. 代码分析+Debug源码
   ...
   ![image-20211201101510728](设计模式.assets/image-20211201101510728.png)

3. 说明

   ![image-20211201101555420](设计模式.assets/image-20211201101555420.png)
...
4. 活动的四个状态转换关系图
  ...
  ![image-20211201102308621](设计模式.assets/image-20211201102308621.png)
...
![image-20211201102405507](设计模式.assets/image-20211201102405507.png)
...
2. 思路分析和图解(类图) -定义出一个接口叫状态接口，每个状态都实现它。 -接口有扣除积分方法、抽奖方法、发放奖品方法
  ...
  ![image-20211201102504931](设计模式.assets/image-20211201102504931.png)
...
2. 通常通过if/else判断订单的状态，从而实现不同的逻辑，伪代码如下
   ...
   ![image-20211201102843819](设计模式.assets/image-20211201102843819.png)
...
1. 传统的设计方案(类图)
   ...
   ![image-20211201104712716](设计模式.assets/image-20211201104712716.png)
...
![image-20211201104951007](设计模式.assets/image-20211201104951007.png)
...
2. 思路分析(类图)
  ...
  ![image-20211201105053722](设计模式.assets/image-20211201105053722.png)
...
2. 代码分析+Debug源码+模式角色分析
   ...
   ![image-20211201105345581](设计模式.assets/image-20211201105345581.png)
...
![image-20211201105943996](设计模式.assets/image-20211201105943996.png)
...
![image-20211201110106744](设计模式.assets/image-20211201110106744.png)
...
1. 应用实例要求
    编写程序完成学校OA系统的采购审批项目：需求 • 采购员采购教学器材
...
2. 思路分析和图解(类图)
   ...
   ![image-20211201110311330](设计模式.assets/image-20211201110311330.png)
...
3. 代码分析+Debug源码+说明
   ...
   ![image-20211201110700004](设计模式.assets/image-20211201110700004.png)
...
6) 所以，提醒大家我们学习设计模式时，（包括看源码分析），要抓住本质，就是使用这个设计模式到底带了了什么好处？ 是 扩展性提高了，还是更加规范了，这样我们才能领会设计模式的精妙之处。
